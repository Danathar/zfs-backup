...
usage() {
  cat <<'USAGE'
Usage:
  zfs_backup_interactive.sh -s SOURCE_POOL -d DEST_POOL -b BASE_SNAP [-n NEW_SNAP] [-e DATASET]... [--dry-run]
...
DEST=""
BASE_SNAP=""
NEW_SNAP="$(date +%Y%m%d-%H%M%S)"
EXCLUDES=()
DRY_RUN=0

# Support long options by converting them into short-option flow first.
while [[ $# -gt 0 ]]; do
  case "$1" in
    --dry-run)
      DRY_RUN=1
...
while getopts ":s:d:b:n:e:h" opt; do
  case "$opt" in
    s) SOURCE="$OPTARG" ;;
    d) DEST="$OPTARG" ;;
    b) BASE_SNAP="$OPTARG" ;;
...
SEND_PIPE_CMD="sudo zfs send -R -I ${SOURCE}@${BASE_SNAP} ${SOURCE}@${NEW_SNAP} --skip-missing | pv | sudo zfs recv -F ${DEST}"
